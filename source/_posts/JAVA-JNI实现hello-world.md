...
public class Native1{
    public int a = 0;
    public native void satA();

    public static void main(String[] args) {
        System.loadLibrary("hellworld");
        Native1 Native1 = new Native1()
        Native1.satA();
...
```
Exception in thread "main" java.lang.UnsatisfiedLinkError: Native1.sayA()V
        at Native1.saya(Native Method)
...
#include <jni.h>
/* Header for class Natives1 */

#ifndef _Included_Natives1
#define _Included_Natives1
#ifdef __cplusplus
extern "C" {
#endif
/*
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_Natives1_setA
  (JNIEnv *, jobject);
...
我们需要在myNative.c(.cpp文件也是支持的, 此文件名称不需要与java类名一致, 只需要动态链接库名与调用的库名有对应关系即可)里面实现Java_Natives1_setA函数:
```
...
JNIEXPORT void JNICALL Java_Native1_sayHello(JNIEnv *env, jobject thisObj)
{
    ...
    // 获取该类中字段a的签名
    jfieldID id_number = env -> GetFieldID(clazz, "a", "I");
    // 获取该字段的值
    jint number = env -> GetIntField(thisObj, id_number);
    printf("%d", number);
    // 修改该该字段的值
    env -> SetIntField(thisObj, id_number, 1000);
}
```
头文件指明了Native1.h。这里主要定义了Java_Natives1_setA函数。
